...
from notifications import scheduler, remove_reminder, schedule_jobs, schedule_vaccination_reminder
import re
import logging
...
async def profile(message: types.Message):
    with Session() as session:
        ...
        for pet in user.pets:
            ...
            await message.answer(
                text,
                reply_markup=get_pet_management_keyboard(pet)
            )
...
async def process_pet_breed(message: types.Message, state: FSMContext):
    ...
    if message.text not in valid_breeds and message.text != "Без породы":
        await message.answer("Пожалуйста, выберите породу из предложенных")
        return

    breed = None if message.text == "Без породы" else message.text
        ...
        if "pet_name" in data:
            with Session() as session:
                ...
                try:
                    ...
                    await query.message.answer(
                        f"✅ Питомец {pet.name} добавлен! Напоминание установлено на сегодня.",
                        reply_markup=get_main_menu()
                    )
                except Exception as e:
            ...
            with Session() as session:
                ...
                if pet:
                    pet.vaccination_date = today
                    session.commit()

                    try:
                        await schedule_vaccination_reminder(
                            query.bot, pet.id, pet.owner.telegram_id,
                            pet.name, today
                        )
                        await query.message.answer(
...
async def save_vaccination_date(message: types.Message, state: FSMContext, new_date: str):
    ...
    with Session() as session:
        ...
        if pet:
            ...
            try:
                await schedule_vaccination_reminder(
                    message.bot, pet.id, pet.owner.telegram_id,
                    pet.name, new_date
                )
                response = f"✅ Дата вакцинации для {pet.name} изменена"
                if old_date:
                    response += f":\nБыло: {old_date}\n"
                response += f"Стало: {new_date}"
                await message.answer(
                    response,
                    reply_markup=get_main_menu()
                )
    ...
    info_texts = {
        "🐱 Кошка": {
            "🍽 Уход": (
                ...
                "• Вода: всегда должна быть свежая\n"
            ...
            "💊 Здоровье": (
                "🐱 <b>Здоровье кошки:</b>\n\n"
                "• Ежегодные прививки\n"
                "• Обработка от паразитов каждые 3 месяца\n"
...
async def back_to_reminders_handler(query: CallbackQuery):
    ...
    with Session() as session:
        ...
        if user and user.pets:
            ...
            if not has_reminders:
                await query.message.answer("У вас пока нет активных напоминаний")
        else:
            await query.message.answer("У вас нет напоминаний")
